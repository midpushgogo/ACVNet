...
class ACVNet(nn.Module):
    ...
    def forward(self, left, right):
        ...
        concat_feature_left = features_left["corr_feature"]
        concat_feature_right = features_right["corr_feature"]



        concat_volume = build_concat_volume(concat_feature_left, concat_feature_right, self.maxdisp // 4)
        ac_volume = F.softmax(F.upsample(att_weights,scale_factor=2,mode='trilinear'), dim=2) * concat_volume   ### ac_volume = att_weights * concat_volume
